...
function ToolCallCard({ toolCall }: { toolCall: ToolCallResult }) {
  ...
  return (
    <div className="flex items-start gap-2 rounded-md bg-slate-100 px-3 py-2 text-xs text-slate-600">
...
function MessageBubble({ message }: { message: AIMessage }) {
  ...
  if (message.isLoading) {
    return (
      <div className="flex justify-start">
        <div className="max-w-[85%] rounded-lg bg-white px-4 py-3 shadow-sm border border-slate-200">
  ...
  if (message.error) {
    return (
      <div className="flex justify-start">
        <div className="max-w-[85%] rounded-lg bg-red-50 px-4 py-3 border border-red-200">
...
export function AIChatPanel({ boardId, isOpen, onClose }: AIChatPanelProps) {
  ...
  return (
    <div
      className={`fixed right-0 top-0 z-50 flex h-full w-96 flex-col border-l border-slate-200 bg-slate-50 shadow-xl transition-transform duration-300 ease-in-out ${
            ...
            <div className="mt-4 flex flex-col gap-2">
              {[
                ...
                <button
                  ...
                  className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600 hover:border-blue-300 hover:bg-blue-50"
                >
                  {suggestion}
                </button>
              ))}
      ...
      >
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800 placeholder-slate-400 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
            disabled={isLoading}
          />
          <button
            type="submit"
            disabled={isLoading || !input.trim()}
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-blue-600 text-white transition-colors hover:bg-blue-700 disabled:bg-slate-300 disabled:cursor-not-allowed"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
